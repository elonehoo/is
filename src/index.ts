...
import type { Class, Falsy, ObservableLike, Primitive, TypedArray, WeakRef } from './type'
...
is.nativePromise = <T = unknown>(value: unknown): value is Promise<T> => isObjectOfType<Promise<T>>('Promise')(value)

const hasPromiseApi = <T = unknown>(value: unknown): value is Promise<T> => is.function_((value as Promise<T>)?.then) && is.function_((value as Promise<T>)?.catch)
...
function assertType(condition: boolean, description: string, value: unknown, options: { multipleValues?: boolean } = {}): asserts condition {
  if (!condition) {
...
export type { Class, TypedArray, ObservableLike, Primitive } from './type'
